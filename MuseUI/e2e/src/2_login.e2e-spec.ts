...
describe('login page', () => {
  ...
  it('should login into the system', () => {
    page.navigateToLogin();
    let newDashboard = page.addLoginValues();
    expect(page.getLoginInputBoxesDefaultValues()).toEqual(newDashboard, 'Should be able to set values for username and password');
    page.clickSubmitButton();
    page.navigateToDashboard();
    page.getCurrentURL().then((url) => {
      if (url.indexOf('login') > -1) {
        newDashboard = page.addLoginValues();
        page.clickSubmitButton();
        page.navigateToDashboard();
        expect(page.getCurrentURL()).toContain('', 'Should navigate to dashboard');
      } else {
        expect(page.getCurrentURL()).toContain('', 'Should navigate to dashboard');
      }
    });
